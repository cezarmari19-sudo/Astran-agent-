...
class SandboxManager:
    ...
    async def run_command(self, project_id: str, command: str) -> dict:
        ...
        The command is wrapped with the container's own `timeout` utility
        rather than relying solely on asyncio.wait_for: asyncio.wait_for
        only cancels the asyncio.Task wrapping asyncio.to_thread — it
        cannot interrupt the underlying blocking docker-py call once it's
        running on a worker thread, so on an asyncio-level timeout the
        real shell command would otherwise keep running inside the
        container indefinitely (a resource leak that also keeps consuming
        the container's CPU/memory quota). Wrapping with `timeout -k` makes
        the container process itself die on schedule regardless of what
        happens on the Python side.
        """
        handle = await self._get_or_create(project_id)
        # -k 5: if it ignores SIGTERM, SIGKILL it 5s later. Runs one extra
        # second past COMMAND_TIMEOUT_SECONDS so asyncio.wait_for below is
        # the one that (almost always) reports the timeout to the caller,
        # while `timeout` guarantees the container-side process still dies
        # even if asyncio.wait_for's own cancellation can't reach it.
        wrapped_command = f"timeout -k 5 {COMMAND_TIMEOUT_SECONDS} /bin/sh -c {_shell_quote(command)}"
        ...
        try:
            exit_code, output = await asyncio.wait_for(
                _exec(), timeout=COMMAND_TIMEOUT_SECONDS + 7
            )
            handle.touch()
            ...
            # exit_code 124 (or 137 if SIGKILL was needed) is `timeout`'s
            # own signal that it killed the command for running too long.
            timed_out = exit_code in (124, 137)
            if timed_out:
                text += f"\n... [comanda a depășit limita de {COMMAND_TIMEOUT_SECONDS}s și a fost întreruptă]"
            return {"exit_code": exit_code, "stdout": text, "timed_out": timed_out, "truncated": truncated}
        except asyncio.TimeoutError:
            # Extremely rare fallback: even the in-container `timeout`
            # wrapper didn't return in time (e.g. Docker daemon itself is
            # stuck). Nothing more we can do from here to reclaim the
            # thread; the reaper will eventually recycle the container.
            return {
                "exit_code": -1,
...
def _shell_quote(s: str) -> str:
    """POSIX single-quote a string for safe embedding in a /bin/sh -c
    command line (used to wrap the user's command with `timeout` without
    reparsing/breaking it)."""
    return "'" + s.replace("'", "'\\''") + "'"
